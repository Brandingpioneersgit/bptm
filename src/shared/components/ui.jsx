    ...
    <div className={className || ''}>
      ...
      <input
        className={`w-full border rounded-xl p-3 text-base focus:ring-2 transition-colors duration-200 touch-manipulation ${
          hasError ? 'border-red-300 focus:ring-red-500 focus:border-red-500 bg-red-50' : hasWarning ? 'border-yellow-300 focus:ring-yellow-500 focus:border-yellow-500 bg-yellow-50' : 'border-gray-300 focus:ring-blue-500 focus:border-blue-500'
  ...
  return (
    <div className={className || ''}>
      ...
      <input
        ...
        inputMode="numeric"
        pattern="[0-9]*"
        className={`w-full border rounded-xl p-3 text-base touch-manipulation ${hasError ? 'border-red-300 focus:ring-red-500 focus:border-red-500 bg-red-50' : hasWarning ? 'border-yellow-300 focus:ring-yellow-500 focus:border-yellow-500 bg-yellow-50' : 'border-gray-300'} ${disabled ? 'bg-gray-100 text-gray-500 cursor-not-allowed' : ''}`}
        value={value ?? ''}
        onChange={e => { if (disabled) return null; const raw = e.target.value; if (raw === '' || raw === null) return onChange(''); return onChange(Number(raw)); }}
        onBlur={onBlur}
        disabled={disabled}
        autoComplete="off"
      />
...
export const TextArea = React.memo(function TextArea({ label, value, onChange, rows = 4, className, placeholder, disabled = false, onBlur }) {
  return (
    <div className={className || ''}>
      ...
      <textarea
        className={`w-full border rounded-xl p-3 text-base resize-y touch-manipulation ${disabled ? 'bg-gray-100 text-gray-500 cursor-not-allowed' : ''}`}
        rows={rows}
        placeholder={placeholder || ""}
        value={value || ""}
        onChange={e => disabled ? null : onChange(e.target.value)}
...
export function MultiSelect({ options = [], selected = [], onChange, placeholder = "Select options", disabled = false, error = null }) {
  ...
  const toggleDropdown = () => { if (disabled) return; setIsOpen(!isOpen); };
  const handleSelect = (option) => { if (disabled) return; if (safeSelected.includes(option)) { onChange(safeSelected.filter(item => item !== option)); } else { onChange([...safeSelected, option]); } };
  useEffect(() => { function handleClickOutside(event) { if (dropdownRef.current && !dropdownRef.current.contains(event.target)) { setIsOpen(false); } } document.addEventListener("mousedown", handleClickOutside); return () => { document.removeEventListener("mousedown", handleClickOutside); }; }, [dropdownRef]);
  const displayValue = safeSelected.length > 0 ? safeSelected.join(", ") : placeholder;
  return (
    <div className="relative" ref={dropdownRef}>
      <button type="button" className={`w-full border rounded-xl p-3 text-left focus:ring-2 transition-colors ${error ? 'border-red-300 focus:ring-red-500 focus:border-red-500 bg-red-50' : disabled ? 'bg-gray-100 text-gray-500 cursor-not-allowed border-gray-300' : 'border-gray-300 hover:border-gray-400 bg-white text-gray-900 focus:ring-blue-500 focus:border-blue-500'} ${safeSelected.length > 0 ? 'text-gray-900' : 'text-gray-500'}`} onClick={toggleDropdown} disabled={disabled} aria-expanded={isOpen} aria-haspopup="listbox">
        <div className="flex justify-between items-center">
          <span className="truncate">{displayValue}</span>
          <span className={`ml-2 transform transition-transform duration-200 ${isOpen ? 'rotate-180' : ''}`}>▼</span>
        </div>
      </button>
      {isOpen && !disabled && (
        <ul className="absolute z-50 w-full bg-white border border-gray-300 rounded-xl mt-1 max-h-60 overflow-y-auto shadow-lg" role="listbox">
          {safeOptions.length === 0 ? (
            ...
            safeOptions.map((option) => (
              <li key={option} onClick={() => handleSelect(option)} className={`p-3 cursor-pointer transition-colors hover:bg-blue-100 ${safeSelected.includes(option) ? 'bg-blue-50 text-blue-900 font-semibold' : 'text-gray-900'}`} role="option" aria-selected={safeSelected.includes(option)}>
                <div className="flex justify-between items-center">
                  <span>{option}</span>
...
export function ProgressIndicator({ current, target, label, unit = "", color = "blue" }) {
  ...
  return (
    <div className="mb-4">
      <div className="flex justify-between items-center mb-2"><span className="text-sm font-medium text-gray-700">{label}</span><span className={`text-sm font-bold ${classes.text}`}>{current}{unit} / {target}{unit}{isComplete && ' ✓'}{isWarning && ' ⚠️'}</span></div>
      ...
      <div className="text-xs text-gray-500 mt-1">{isComplete ? '✓ Target achieved!' : isWarning ? `⚠️ ${(target - current).toFixed(1)}${unit} needed to reach target` : `${(target - current).toFixed(1)}${unit} remaining`}</div>
    </div>
  );
}

export function ThreeWayComparativeField({ label, currentValue, previousValue, comparisonValue, onChange, placeholder = "Enter value", unit = "", disabled = false, monthComparison = "Baseline Month", monthPrev = "Previous Month", monthThis = "This Month" }) {
...
// Displays validation state for a multi-field step: lists errors and warnings
    ...
    return (
      <div className="flex items-center gap-2 p-3 rounded-lg bg-green-50 border border-green-200 text-green-700">
        ...
        <div className="text-sm">
          <span className="font-medium">{stepTitle} looks good.</span>
        </div>
      </div>
    );
  }
  return (
    <div className="p-3 rounded-lg bg-yellow-50 border border-yellow-200 text-yellow-700">
      <div className="flex items-center gap-2 mb-2">
        <span>⚠️</span>
        <span className="font-medium">Please review before proceeding</span>
      </div>
      {hasErrors && (
        <div className="mb-2">
          <div className="text-red-700 font-medium mb-1">Errors</div>
          <ul className="list-disc pl-5 text-sm text-red-700 space-y-1">
            {errors.map((e, i) => (<li key={`err-${i}`}>{e}</li>))}
          </ul>
        </div>
      )}
      {hasWarnings && (
