...
async def run_test():
    ...
    try:
        ...
        # Click the Login button to start login process as Super Admin
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div/div/div/div/div/div/div[2]/button[3]').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # Input First Name and Phone Number, then click Sign In button
        frame = context.pages[-1]
        ...
        # Check for alternative login options or verify correct Super Admin credentials
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        assert False, 'Test plan execution failed: export verification could not be completed.'
