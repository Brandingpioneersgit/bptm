...
async def run_test():
    ...
    try:
        ...
        # Assert that the error message indicating invalid credentials is displayed
        error_message_locator = frame.locator('xpath=//div[contains(text(), "No user found with name starting with")]')
        await error_message_locator.wait_for(state='visible', timeout=5000)
        error_text = await error_message_locator.text_content()
        assert "No user found with name starting with \"Admin Super\"" in error_text
        # Assert that the dashboard or welcome message is not visible, indicating login failure
        welcome_message_locator = frame.locator('text=Welcome Back')
        assert await welcome_message_locator.count() == 0
        page_title = await frame.title()
        assert page_title != "BP Agency Dashboard"
